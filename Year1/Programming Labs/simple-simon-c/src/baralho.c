...
void baralho_inicializar(Baralho *b) { //*b para modificar o baralho original senão mexiamos numa copia, se andasse a passar valor iamos copiar 52 cartas a cada chamada
    ...
    for (int naipe = 0; naipe < 4; naipe++) { // existe 4 naipes
        for (int valor = 1; valor <= 13; valor++) { // cada naipe 13 cartas
...
int baralho_vazio(const Baralho *b) { //importante pq o jogo termina quando o baralho termina
